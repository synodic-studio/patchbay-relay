...
| `/compact [steering]` | Trigger claude's `/compact` slash command on the live session, optionally steered by trailing text. Reports before/after token counts: `Context compacted: 40.6k → 33.8k`. cc-sdk only. Requires an active session (otherwise nothing to compact). |
