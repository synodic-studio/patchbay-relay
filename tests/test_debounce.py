...
@pytest.mark.asyncio
async def test_inflight_push_routes_message_into_running_cc_sdk_session():
    """When a cc-sdk turn is mid-stream, a second message must NOT be
    queued — it must be pushed into the live ClaudeSDKClient via
    `ClaudeSdkHarness.push()` on the worker loop. User sees no
    "Queued (1)" reply because the running session is going to handle
    it inline.

    Regression: before inflight wiring, every second message landed in
    the bridge queue and got run as a fresh subprocess after the first
    one finished — a separate Claude invocation, separate session
    state, separate Telegram delivery. This test pins the new path:
    state.harness.push is called, queue stays empty, no reply text.
    """
    from patchbay.harness import ClaudeSdkHarness

    key = bridge._session_key(1, None)
    state = bridge._get_session_state(key)
    state.processing = True

    # The bridge's worker loop is captured inside _drive_harness_sync;
    # in this unit test we point it at the running asyncio loop so
    # run_coroutine_threadsafe can dispatch onto it.
    state.worker_loop = asyncio.get_running_loop()

    # Stand up a ClaudeSdkHarness with a fake live client. push() returns
    # True iff _live_client is set, so the cheapest stand-in is a Mock
    # with an awaitable .query.
    harness = ClaudeSdkHarness()
    fake_client = MagicMock()
    fake_client.query = AsyncMock()
    harness._live_client = fake_client
    state.harness = harness

    update = _make_update(text="follow-up while running")
    ctx = _make_context()

    with patch.object(bridge, "save_pending", return_value="p-inflight"), \
         patch.object(bridge, "clear_pending") as clear_pending:
        await bridge.handle_message(update, ctx)

    # The pushed message reached the SDK client (one query call with the
    # exact text).
    fake_client.query.assert_awaited_once_with("follow-up while running")
    # And the bridge cleared the pending file once push succeeded —
    # the SDK owns the message now; replay_pending() must not also fire it.
    clear_pending.assert_called_once_with("p-inflight")
    # No queue entry was added — that's the whole point of inflight push.
    assert not state.queue
    # User got no "Queued" reply (the running session is going to reply).
    update.message.reply_text.assert_not_called()
    # And the bridge bookkeeping advanced inflight queries on the harness.
    assert harness._inflight_queries == 1


@pytest.mark.asyncio
async def test_inflight_push_falls_back_to_queue_when_harness_has_no_live_client():
    """If state.harness is a ClaudeSdkHarness but no turn is currently
    streaming (e.g. between turns, or the SDK already returned), push()
    declines and the bridge MUST fall through to the existing queue
    path — otherwise the message vanishes. The user sees the normal
    'Queued (1)' reply.
    """
    from patchbay.harness import ClaudeSdkHarness
    ...
    state.processing = True
    state.worker_loop = asyncio.get_running_loop()
    # Harness present but no live client — push() returns False.
    state.harness = ClaudeSdkHarness()

    update = _make_update(text="should be queued")
    ctx = _make_context()

    with patch.object(bridge, "save_pending", return_value="p-q"):
        await bridge.handle_message(update, ctx)

    assert [item.text for item in state.queue] == ["should be queued"]
    update.message.reply_text.assert_called_once()
    assert "Queued (1)" in update.message.reply_text.call_args[0][0]
