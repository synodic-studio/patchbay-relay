...
`asyncio.sleep(0)` — the latter was the source of the historical
`test_queued_messages_drained_after_processing` flake (audit §18).

Per `docs/STARGATE-IMPROVEMENT-PLAN.md` §3a:
> One integration test that (a) starts handler A via a task, (b) sends
> handler B a second message via the real Telegram handler API, (c)
> asserts both replies land and in order.
"""
