...
Covers the debounce race documented in STARGATE-IMPROVEMENT-PLAN §1c
(CTB-ucw). Two messages arriving in the same event-loop tick must produce
exactly one "claimed" and the rest "queued" — without the asyncio.Lock,
both could pass the `if state.processing` check and stomp on each other.
"""
