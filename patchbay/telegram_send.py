...
async def _send_response(bot, chat_id: int, thread_id: int | None, response: str) -> None:
    ...
    # Runs AFTER the silence/noisy drops so MOP only sees what is actually
    # delivered — the audit means "what the user got", not "what the agent
    # emitted". Fail-open: MOP must never withhold a response on its own error.
    if response:
        try:
