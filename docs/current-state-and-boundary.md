...
Owner lean (2026-07-08): since Claude Code cannot be leaned on substantially under the subscription going forward, the likely direction is to move even self-heal (and drop `/usage`) toward pi rather than keep a Claude-shaped periphery alive. Not yet decided, and it cuts against the "heal with a different engine" argument above. Flagged hard because self-heal is load-bearing: if it silently depends on a `claude` invocation the subscription no longer funds, the bridge's core safety net degrades exactly when Relay is being reactivated. Settle this deliberately, do not let it rot.
