...
def _project_path(entry) -> str | None:
    """Extract the relative path string from a chat_projects entry.

    Entries are either a plain string ("Fanta") or a dict ({"path": "Fanta", "agent": "..."}).
    """
...
def get_chat_working_dir(session_key: str) -> str:
    ...
    rel_path = _project_path(projects.get(session_key))
    if rel_path:
...
def _parse_events(stdout: str) -> list[dict]:
    """Parse stdout as JSON array or NDJSON lines into a list of dicts."""
...
def parse_claude_response(stdout: str, session_key: str) -> str:
    ...
    if result_event:
        ...
        if new_session_id and not result_event.get("is_error"):
            ...
            logger.warning(
                "Skipped saving error session %s for %s",
...
def run_claude(message: str, session_key: str) -> str:
    ...
    system_prompt = (
        ...
        "Bryan's accounts: iCloud (bryancostanza@icloud.com) and Gmail (kj6dev@gmail.com). "
        ...
        "~/Library/CloudStorage/ProtonDrive-merganser538@pm.me-folder/Claude-Support. "
    ...
    # Check for stale session error in JSON output (mirrors the stderr check above)
    if session_id and proc.returncode != 0:
        for e in _parse_events(stdout):
            if e.get("type") == "result" and e.get("is_error"):
                errors = e.get("errors", [])
                if any("No conversation found" in str(err) for err in errors):
                    logger.warning(
                        "Stale session %s for %s (JSON), retrying fresh",
                        session_id[:12], session_key,
                    )
                    clear_session(session_key)
                    return run_claude(message, session_key)
...
async def cmd_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    key = _session_key(chat_id, thread_id)
    projects = _load_chat_projects()
    rel_path = _project_path(projects.get(key))
    if rel_path:
...
async def cmd_selftest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Run validation checks and report results without restarting."""
    user_id = update.effective_user.id
    if ALLOWED_USER_IDS and user_id not in ALLOWED_USER_IDS:
        return

    passed, output = _run_validation()
    if passed:
        await update.message.reply_text(f"Validation passed: {output}")
    else:
        await update.message.reply_text(f"Validation FAILED:\n\n{output}")


async def cmd_restart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Restart the bridge process. Validates code first, then launchd respawns."""
    user_id = update.effective_user.id
    if ALLOWED_USER_IDS and user_id not in ALLOWED_USER_IDS:
        return
    ...
    if not passed:
        await update.message.reply_text(
            f"RESTART BLOCKED — validation failed:\n\n{output}\n\n"
            "Fix the issue and try /restart again. Bridge continues running on current code."
        )
        logger.warning("Restart blocked: validation failed: %s", output)
        return

    # Save current working bridge.py as known-good backup
    script_dir = Path(__file__).parent
    import shutil
    shutil.copy2(script_dir / "bridge.py", script_dir / ".bridge-known-good.py")
    logger.info("Saved known-good backup of bridge.py")

    await update.message.reply_text("Validation passed. Restarting bridge...")
    logger.info("User %d triggered bridge restart (validation passed)", user_id)
...
async def post_init(app: Application) -> None:
    ...
    await app.bot.set_my_commands(
        [
            ...
            BotCommand("selftest", "Run validation checks"),
